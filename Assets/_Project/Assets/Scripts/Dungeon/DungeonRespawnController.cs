...
//           (Health 를 거치지 않으므로 사망 연출도, 게임오버도 없다).
...
public class DungeonRespawnController : MonoBehaviour {
    ...
    public static DungeonRespawnController Begin(GameObject player, float fallYThreshold, int fallDamage, Action onPlayerDeath) {
        ...
        controller.Setup(player, fallYThreshold, fallDamage, onPlayerDeath);
        return controller;
    }
    ...
    void Setup(GameObject player, float fallYThreshold, int fallDamage, Action onPlayerDeath) {
        Instance = this;
    ...
    public void RespawnAtCurrentRoom() {
        ...
        if (fallDamage > 0 && playerHealth != null) playerHealth.TakeDamage(fallDamage, anchor.position);
    }
